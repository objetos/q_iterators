...
To yield only a subset of cells, use the `quadrille` [cells(filter)]({{< relref cells >}}) method within the loop. The [visit(callback, filter?)]({{< relref visit >}}) method offers a concise alternative by applying a `callback` function to each yielded cell.
